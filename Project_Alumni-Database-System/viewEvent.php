<?php 
	...
	$disableImageSlider = "style=\"display: none;\"";
	...
	function getImagesBasedOnId($eventId, $querieslist) {
		...
		$result = count($GLOBALS['imagesDescArray']);
		
		if ($result > 0) {
			$GLOBALS['disableImageSlider'] = "";
		}
...
<link href="js/imageSlider/themes/4/js-image-slider.css" rel="stylesheet" type="text/css" />
<script src="js/imageSlider/themes/4/js-image-slider.js" type="text/javascript"></script>
...
<body>
    ...
    <div id="sliderFrame" <?= $disableImageSlider;?>>
        <div id="slider">
		  <?php	  
		  for ($i = 0; $i < count($GLOBALS['imageFileNameArray']); ++$i) {
			?>
			<img src="<?= $GLOBALS['imageFileNameArray'][$i] ?>"  />
		  <?php
			}			
		  ?>
        </div>
    ...
    <BR>
		<table align="center">
			...
			<form action="./donateEvent.php?eventId=<?= $eventId;?>" method="get">
					<td><input name="leaveCommentButton" type="submit" class="btn btn-primary" formmethod="post" formaction="./insertcomment.php?eventId=<?= $eventId;?>" value="Leave Comment" /></td>			
				</form>
				...
				<?php 
					mysqli_stmt_close($stmt);
					$querieslist = getQueriesListXML('ViewEvent.query');
					$stmt = $dbc->prepare($querieslist->CHECK_IF_LOGGED_IN_MEMBER_IS_FR_OWNER->SQLStatement);
					$stmt->bind_param( "i", $eventId);
					
					$stmt->execute();
					
					$stmt->bind_result($fr_event_owner);
					...
					mysqli_stmt_close($stmt);
					$querieslist = getQueriesListXML('ViewEvent.query');
					$stmt = $dbc->prepare($querieslist->CHECK_IF_LOGGED_IN_MEMBER_IS_NFR_OWNER->SQLStatement);
					$stmt->bind_param( "i", $eventId);
						
					$stmt->execute();
						
					$stmt->bind_result($nfr_event_owner);
					/* if (mysqli_stmt_fetch($stmt)){
					 echo '<option value='.$col1.'>'.$col2.'</option>';
					} */
					mysqli_stmt_fetch($stmt);
					mysqli_stmt_close($stmt);
					closeDBConnection($dbc);	
					if($fr_event_owner==$_SESSION["LoginUserID"]||$nfr_event_owner==$_SESSION["LoginUserID"]){
						echo '<td><input name="updateEvent" id="updateEvent" type="button" class="btn btn-primary" value="Update Event" style="margin-right:0"/></td>';
						echo '<td><input name="uploadPhotos" id="uploadPhotos" type="button" class="btn btn-primary"  value="Upload Event Photos"/></td>';
					}
				?>
